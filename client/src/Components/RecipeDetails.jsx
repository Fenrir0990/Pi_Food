        ...
        <div className="BackGraundDetails">
            { load?
                ...
                <div key="content" className="con">
                    <img src={recipe?.image} key="imgfood" className="imgRecipe"  alt="Img Not Found"/>
                    <div  key="ab"className="contained_info">
                        <div className="info">
                            <h2>name:{recipe.title?recipe.title:"cargando titulo"}</h2>
                            <h3>{recipe.summary?recipe.summary:null}</h3>
                            <h4>{recipe.instructions?recipe.instructions:null}</h4>
                             {recipe.diets?.map((dieta)=><button  key = {dieta.name}className="dieta">{dieta.name}</button>)}
